...
)
{
    function(rc_exdf) {
        ...
        # To estimate Rd, first make a linear fit of A ~ Cc where Cc is below the
        # threshold. Then, evaluate the fit at Cc = Gamma_star. Gamma_star is
        # temperature dependent and not necessarily constant across the measured
        # points, so use its average value across points where Cc is below the
        # threshold. If there are not enough points to do the fit, just estimate Rd
        ...
        # To estimate Vcmax, we solve Equation 2.20 for Vcmax and calculate it for
        # each point in the response curve. Vcmax values calculated this way should
        # be similar when the leaf is in the rubisco-limited range. When the leaf
        # enters the RuBP-regeneration-limited range, the measured values of A will
        # generally be smaller than the Rubisco-limited ones, and the calculated
        # values of Vcmax will be smaller than the ones from the Rubisco-limited
        # range. With this in mind, we choose the largest Vcmax value as our best
        # estimate. In this calculation, we need a value of Rd, so we use the
        ...
        # point in the response curve. Then we choose the largest value as the best
        # estimate.
        j_estimates <- Ag *
        ...
        # To estimate TPU, we solve Equation 2.26 for TPU and calculate it for each
        # point in the response curve. Then we choose the largest value as the best
        # estimate.
        tpu_estimates <- Ag *
