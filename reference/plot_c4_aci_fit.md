...
- a_column_name:
  ...
  value that was passed to `fit_c4_aci`.

- ci_column_name:

  The name of the columns in the elements of `fit_results` that contain
  the intercellular CO2 concentration in `micromol mol^(-1)`; should be
  the same value that was passed to `fit_c4_aci`.

- pcm_column_name:

  The name of the columns in the elements of `exdf_obj` that contain the
  partial pressure of CO2 in the mesophyll, expressed in `microbar`.
...
The resulting plot will show curves for the fitted rates `An`, `Apr`,
`Apc`, and `Ar`, along with points for the measured values of `A`, and
