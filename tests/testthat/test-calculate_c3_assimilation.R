...
test_that('c3 assimilation works for numeric values of flexible inputs', {
    ...
    res <- expect_silent(
        calculate_c3_assimilation(inputs2, 0, 0, 0, 0, 40, 150, 1, 12, 120)
    )
...
test_that('alpha restrictions are enforced', {
    # All alphas nonzero
    expect_error(
        calculate_c3_assimilation(inputs, 0.1, 0.1, 0.1, 0.1, 40, 150, 1, 12, 120),
    ...
    expect_error(
        calculate_c3_assimilation(inputs, 0.0, 0.1, 0.1, 0.0, 40, 150, 1, 12, 120),
        'Cannot specify nonzero alpha_old and nonzero alpha_g / alpha_s / alpha_t',
        fixed = TRUE
    )
    ...
    expect_error(
        calculate_c3_assimilation(inputs, 0.1, 0.1, 0.0, 0.0, 40, 150, 1, 12, 120),
        'Cannot specify nonzero alpha_old and nonzero alpha_g / alpha_s / alpha_t',
        fixed = TRUE
    )
    ...
    expect_error(
        calculate_c3_assimilation(inputs, 0.5, 0.0, 0.5, 0.5, 40, 150, 1, 12, 120, hard_constraints = 2),
        'alpha_g + 2 * alpha_t + 4 * alpha_s / 3 must be <= 1',
        fixed = TRUE
    )

    expect_silent(
        calculate_c3_assimilation(inputs, 0.5, 0.0, 0.5, 0.5, 40, 150, 1, 12, 120)
    )

    # alpha_g and alpha_s nonzero and atp_use not 4
    expect_error(
        calculate_c3_assimilation(inputs, 0.5, 0.0, 0.1, 0.0, 40, 150, 1, 12, 120, atp_use = 4.5),
    ...
    expect_error(
        calculate_c3_assimilation(inputs, 0.5, 0.0, 0.1, 0.0, 40, 150, 1, 12, 120, nadph_use = 10.5),
        'atp_use must be 4 and nadph_use must be 8 when alpha_g / alpha_s / alpha_t are nonzero',
        fixed = TRUE
    )
    ...
    expect_error(
        calculate_c3_assimilation(inputs, 0.5, 0.0, 0.1, 0.0, 40, 150, 1, 12, 120, atp_use = 4.5, nadph_use = 10.5),
        'atp_use must be 4 and nadph_use must be 8 when alpha_g / alpha_s / alpha_t are nonzero',
        fixed = TRUE
    )
})
...
test_that('fitting parameter limits can be bypassed', {
    expect_error(
        calculate_c3_assimilation(inputs, -1, -1, -1, 0, 40, 150, 1, 12, 120, hard_constraints = 2),
        'alpha_g must be >= 0 and <= 1. alpha_old must be >= 0 and <= 1. alpha_s must be >= 0 and <= 1',
        fixed = TRUE
    )

    expect_silent(
        calculate_c3_assimilation(inputs, -1, -1, -1, 0, -40, -150, -1, -12, -120, hard_constraints = 1)
    )

    expect_error(
        calculate_c3_assimilation(inputs3, -1, -1, -1, 0, 40, 150, 1, 12, 120, hard_constraints = 1),
        'Cc must be >= 0'
    )
})
...
test_that('Cc limits can be bypassed', {
    expect_error(
        calculate_c3_assimilation(inputs3, 0, 0, 0, 0, 40, 150, 1, 12, 120, hard_constraints = 2),
        'Cc must be >= 0'
    )

    expect_error(
        calculate_c3_assimilation(inputs3, 0, 0, 0, 0, 40, 150, 1, 12, 120, hard_constraints = 1),
        'Cc must be >= 0'
    )
