...
assim <- calculate_c3_assimilation(inputs, alpha_g, '', J, Rd, Tp, Vcmax)
...
to a measured curve, it is possible to estimate values for `Tp`, `J`, `Rd`, and
...
parameters: `Tp`, `J`, `Rd`, and `Vcmax`. If `fixed` is not specified, it will
take its default value: `c(Inf, NA, NA, NA)`. In that case, `Tp` would be fixed
...
rate `Ap` is determined by the `Tp` parameter; for some of the curves, the
...
apparent that the `Tp` issue remains, and some curves are never
...
`Tp` to a high value and only fitting `J`, `Rd`, and `Vcmax`. Often it is wise
to first try fitting a set of curves without `Tp` limitations, and only
consider allowing `Tp` limitations if the resulting fits are not good.
