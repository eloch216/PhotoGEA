...
assim <- calculate_c3_assimilation(inputs, alpha_g, alpha_old, alpha_s, '', J, RL, Tp, Vcmax)
...
to a measured curve, it is possible to estimate values for `Tp`, `J`, `RL`, and
...
parameters: `Tp`, `J`, `RL`, and `Vcmax`. If `fixed` is not specified, it will
...
assimilation, and only `J`, `RL`, and `Vcmax` would be varied.
...
estimated operating point. We can take a look at these using a built-in plotting
function:
...
columns_for_viewing <-
  c('instrument', 'species', 'plot', 'Vcmax_at_25', 'J_at_25', 'RL_at_25', 'Tp')
...
`Tp` to a high value and only fitting `J`, `RL`, and `Vcmax`. Often it is wise
