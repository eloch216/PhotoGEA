...
assim <-
  calculate_c3_assimilation(inputs, alpha_g, alpha_old, alpha_s, alpha_t, '', J, RL, Tp, Vcmax)
...
`alpha_old` = `r alpha_old`, `J` = `r J` `r assim$units$J_tl`,
`RL` = `r RL` `r assim$units$RL_tl`, `Tp` = `r Tp` `r assim$units$Tp`,
`Vcmax` = `r Vcmax` `r assim$units$Vcmax_tl`, and a leaf temperature of
`r Tleaf` `r inputs$units$Tleaf`. Temperature response parameters from
@sharkey_fitting_2007 were used for these calculations.)
...
file to calculate values of several key C~3~ photosynthetic parameters
