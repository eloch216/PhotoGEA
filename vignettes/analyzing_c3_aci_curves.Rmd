...
shortened to the _FvCB_ model or _Farquhar_ model) after the three scientists
...
- At high levels of CO~2~ in the chloroplast, CO~2~ assimilation is primarily
  limited by phosphate utilization.

More specifically, the model provides equations that separately describe the
...
The overall net assimilation rate $A_n$ at a particular value of the
chloroplastic CO~2~ concentration is then taken to be the smallest of the three
individual limiting rates. Using the equations that compose this model, it is
possible to calculate plots like the one below, which illustrates the three
different assimilation ranges.
...
the `PhotoGEA` package, and it represents the photosynethetic response of a C~3~
plant according to the FvCB model with `TPU` = `r TPU` `r assim$units$TPU`,
...
RuBP-regeneration-limited, and phosphate-limited assimilation predicted by the
model are rarely observed in practice, and there are instead regions of
co-limited assimilation. This can be accomodated by an empirical "curvature"
parameter, as discussed in the documentation for `calculate_c3_assimilation`.
The following figure shows a CO~2~ response curve calculated with the curvature
set to `r curvature` and other parameters identical to the ones used to generate
the previous figure.

```{r, echo = FALSE}
assim_curvature <-
  calculate_c3_assimilation(inputs, TPU, J, Rd, Vcmax, curvature = curvature)
...
values of the chloroplast CO~2~ concentration $C_c$ and temperature-dependent
values of important photosynthetic parameters such as $\Gamma^*$. However, the
`PhotoGEA` package includes two functions to help with these calculations:
`calculate_cc` and `calculate_arrhenius`. Each of these requires an `exdf`
...
`?calculate_cc` or `?calculate_arrhenius`.

Another important consideration is that `calculate_cc` requires values of the
...
xyplot(
  ...
  auto = TRUE,
  grid = TRUE,
  xlab = paste0('Intercellular CO~2~ concentration (', c3_aci_results$fits$units$Ci, ')'),
  ylab = paste0('Net CO~2~ assimilation rate (', c3_aci_results$fits$units$A, ')')
)
```

Note that in addition to the fitted values of `A`, the output also includes
values of the individual limiting rates `Ac`, `Aj`, and `Ap`. We can also take a
...
less reliable than others. For example, the phosphate-limited rate `Ap` is
determined by the `TPU` parameter; for some of the curves, the assimilation rate
is never limited by `Ap`, so the resulting "best-fit" values of `TPU` may not be
meaningful for these curves.

There are also two curves (`ripe2 - soybean - 1` and `ripe2 - tobacco - 4`) that
do not follow the expected progression from rubisco-limited assimilation to
electron-transport-limited assimilation as `Cc` increases; this entire fit may
be unreliable, even if `A_fit` closely matches `A`. In fact, it seems that the
optimizer is overestimating `J`, so `Aj` is never the limiting factor in these
curves. To address this, it is possible to force the optimizer to only allow
fits where `Aj` is less than `Ac` above a certain threshold value of `Cc`;
adding this restriction can sometimes produce more reasonable fits.
...
Here we can see improvements in the fits for `ripe2 - soybean - 1` and
`ripe2 - tobacco - 4`. The fit for `ripe1 - soybean - 5a` is a little strange,
but based on the values of `Ci` and `Cc`, it seems that this plant's stomata did
not fully open during the measurement, almost fully restricting the measured
assimilation values to the rubisco-limited range; there is nothing that can be
done about this. It is also apparent that the `TPU` issue remains, and some
curves are never phosphate-limited; again, there is nothing that can really
