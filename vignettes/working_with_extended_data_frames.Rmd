...
can supply new units and categories using the `specify_variables` function:

```{r}
exdf_2 <- specify_variables(exdf_2, c('Cat1', 'A', 'm'), c('Cat2', 'B', 's'))
```
...
were automatically created by the `read_licor_file` function. Any of these
...
`specify_variables` as in [Initializing with Default Units and Categories].
...
column called `D` using the following code:

```{r}
exdf_2$main_data$D <- 17
```

Now, `exdf_2` is no longer a properly defined `exdf` object because there is a
`D` column in `exdf_2$main_data` that is not present in `exdf_2$units` or
`exdf_2$categories`. This may prevent other functions from working properly; for
example, `print` will not properly display the `D` column. This is a subtle
...
2. Use `lapply` and `read_licor_file` to load each file, producing a `list` of
   `exdf` objects.
...
_Analyzing Ball-Berry Data_ vignette.
...
additional gas properties and the Ball-Berry index:
...
by_result <- by(
  ...
  'gsw', 'bb_index'                                    # additional function arguments
)
```
