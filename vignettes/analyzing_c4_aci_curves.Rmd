...
The `fit_c4_aci` function can estimate values of $J$, but not $J_{max}$.
However, estimates of $J_{max}$ can be obtained after fitting by applying the
...
done for the previous fits.

In order to do this, there are a few required pieces of information that are not
included in the results: the temperature-dependent values of $\alpha_J$ and
$\theta_J$, two parameters related to $J$ and $J_{max}$. These can be obtained
using the `calculate_temperature_response` function, which was also used above
in a different context. For more information about these functions, see the
built-in help menu entries by typing `?calculate_jmax` or
