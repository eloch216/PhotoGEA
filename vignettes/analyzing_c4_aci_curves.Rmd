...
rate of PEP carboxylase regeneration ($V_{p,r}$), the maximum electron transport
rate ($J_{max}$), the total rate of mitochondrial respiration across the
mesophyll and bundle sheath cells ($R_L$), the partial pressure of CO~2~ in the
...
assim <- calculate_c4_assimilation(
  inputs,
  alpha_psii, gbs, Jmax_at_opt, RL_at_25, Rm_frac, Vcmax_at_25, Vpmax_at_25, Vpr
)
...
`Jmax_at_opt` = `r Jmax_at_opt` `r assim$units$Jmax_at_opt`, and a leaf
...
$V_{cmax}$, $J_{max}$, and others. See the documentation for
...
functions to help with these calculations: `calculate_temperature_response_arrhenius`,
`calculate_temperature_response_gaussian`, `calculate_total_pressure`, and `apply_gm`. Each of
...
by typing `?calculate_temperature_response_arrhenius`, `?calculate_total_pressure`, or `?apply_gm`.

First, we can use `calculate_temperature_response_arrhenius` and `calculate_temperature_response_gaussian`:

```{r calculate_t_response}
# Calculate temperature-dependent values of C4 photosynthetic parameters
licor_data <- calculate_temperature_response_arrhenius(licor_data, c4_arrhenius_von_caemmerer)

licor_data <- calculate_temperature_response_gaussian(licor_data, c4_peaked_gaussian_von_caemmerer)
```
...
_Setaria viridis_ and may not be appropriate for all C~4~ plants. Nevertheless,
...
high $C_i$, we can set $V_{pr}$ and $J_{max}$ to extremely high values, which
...
we will allow $J_{max}$ to vary during the fit. This can be done using the
...
Estimates of other parameters, such as $V_{cmax}$, $J_{max}$, $V_{max}$, and
