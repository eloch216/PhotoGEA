...
assim <- calculate_c4_assimilation(inputs, Rd, Vcmax, Vpmax, Vpr)
...
C~4~ leaf according to the model with `Rd` = `r Rd` `r assim$units$Rd_tl`,
`Vpmax` = `r Vpmax` `r assim$units$Vpmax_tl`,
...
$\gamma^*$ and values of the partial pressure of CO~2~ in the mesophyll
$P_{cm}$. However, the `PhotoGEA` package includes two functions to help with
these calculations: `calculate_arrhenius` and `apply_gm`. Each of these requires
an `exdf` object containing Licor data. The units for each required column will
be checked in an attempt to avoid unit-related errors. More information about
these functions can be obtained from the built-in help system by typing
`?calculate_arrhenius` or `?apply_gm`.
...
Now we can use `apply_gm` to calculate `PCm`:
...
values of the individual limiting rates `Apc` and `Ar`. We can also take a look
at these, although they make the plots a bit busy. We can also choose to plot
