...
With this simple command, we have used the leaf temperature from the Licor
...
This information is not typically included in a Licor file, but we calculated
average values of $Q_{in}$ and $T_{leaf}$ in the [Reorganizing the Data] section
when calling `organize_response_curve_data`, so this is taken care of.
...
[Fitting Licor Data] section above.
...
Here we used constant values of $\theta_J = 0.7$ and $\alpha_J = 0.85 \cdot
(1 - 0.15) \cdot 0.5 = 0.36125$, as recommended in @von_caemmerer_updating_2021.
