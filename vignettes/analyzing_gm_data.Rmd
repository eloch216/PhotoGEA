...
The isotopic compositions can be expressed relative to an international standard,
making them the preferred way to express the relative abundance of different
isotopes.

During photosynthetic carbon isotope discrimination measurements, it is not
typically possible to actually measure $\delta^{13}C_p$ for a variety of reasons.
Instead, $\delta^{13}C$ can be measured in the outgoing air from the leaf.
Then, it is possible to relate the isotopic compositions of the incoming and
outgoing air to and from the leaf ($\delta^{13}C_{in}$ and $\delta^{13}C_{out}$,
respectively) to $\delta^{13}C_p$. In this case, the full equation for
$\Delta^{13}C$ becomes $$\Delta^{13}C = \frac{\xi (\delta^{13}C_{out} -
\delta^{13}C_{in})}{1 + \delta^{13}C_{out} - \xi (\delta^{13}C_{out} -
\delta^{13}C_{in})}.$$ Here, $\xi$ is defined by $$\xi = \frac{C_{in}}{C_{in} -
C_{out}},$$ where $C_{in}$ and $C_{out}$ are the ^12^C concentrations in the
...
Later, it will be necessary to know the oxygen concentration of the ambient air
in the gas exchange measurement cuvette. This information is included in the
preamble of each Licor file. We can extract the value from the preamble of each
file and store it as a column using the `get_oxygen_from_preamble` function from
the `PhotoGEA` package. This is a more convenient way to include the oxygen
concentration.

```{r oxygen_concentration}
# Get oxygen info from each Licor file
licor_exdf_list <- lapply(licor_exdf_list, get_oxygen_from_preamble)
```
...
# Loading Tunable Diode Laster Data
...
check_licor_data(licor_data, 'curve_identifier', 12, 'CO2_r_sp')
```
...
check_licor_data(licor_data, 'curve_identifier', 13)

check_licor_data(licor_data, 'replicate', 12)
```
...
Like, the observed photosynthetic isotope discrimination under growth conditions
($\Delta_{obs}^{growth}$) must be measured from each leaf or plant species.
Ideally, this would be determined by setting the ambient CO~2~ concentration
around the leaf to the ambient value and using the TDL to measure the
photosynthetic isotope discrimination $\Delta_{obs}$. In this data set, the
