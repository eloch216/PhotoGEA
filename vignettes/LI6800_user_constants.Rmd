...
The first three sections of this vignette provide key advice about how to set up
user constants and what they can be used for:

1. [Benefits of User Constants] -- A brief overview of the benefits of user
   constants.

2. [Choosing User Constants] -- Advice for choosing user constants, including
   examples.

3. [Using User Constants] -- Advice for setting up user constants for
   measurements.

The later sections include more information about a few related topics:

4. [Using File Names or Log Remarks] -- A discussion of alternatives to user
   constants; this section illustrates why user constants are generally simpler
   and easier.

5. [How To Deal With Files That Have No Metadata] -- A discussion of what to do
   with log files where user constants, file names, or log remarks were not
   used to record metadata.

6. [Examples of User Constant Applications] -- A few short examples of using
   user constants when processing and analyzing data, building off the brief
   overview in the [Benefits of User Constants] section.

Throughout this vignette, key pieces of advice will be prefaced with
"**Advice**." The final section in the vignette ([Advice Summary]) contains a
list of all such pieces of advice.
...
# Benefits of User Constants

Simply put, including user constants in your data will make processing and
analyzing the data much easier. There are two main ways that user constants can
help:
...
caption <-
  ...
  `consolidate`. Here the `id` column identifies each individual curve, and the
  `ring` value indicates whether each leaf was grown under ambient (A) or
  elevated (E) carbon dioxide conditions. The processing function function fits
  a single A-Ci curve and returns a list of two tables called `fits` and
  `parameters`.'
...
This is a great place to start, but it's missing some important details, which
are provided below.
...
advice(INCLUDE_PREFIX, 'Be consistent with spelling and capitalization.')
```
...
There is no general way to do this, but we discuss a few options below.
...
**Advice for Using Licor LI-6800 User Constants**
