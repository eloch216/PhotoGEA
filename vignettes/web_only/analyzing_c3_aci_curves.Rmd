...
In this vignette, we will give an example showing how to analyze C~3~ _A-C~i~_
...
to be based around Rubisco kinetics, describing how the net assimilation rate
...
scientists who first published it. An excellent description of this model can be
found in _Biochemical Models of Leaf Photosynthesis_
[@caemmerer_biochemical_2000], subject to a few additional considerations
discussed in @lochocki_widely_2025.

This model provides a framework for understanding the changes in $A_n$ that
occur as a C~3~ leaf is exposed to successively higher concentrations of CO~2~.
Overall, the response to CO~2~ under high light conditions can be divided into
...
- At intermediate levels of CO~2~ in the chloroplast, CO~2~ assimilation is
  primarily limited by the rate of electron transport going to support RuBP
  regeneration; the potential rate of electron transport at a given incident
  PPFD is denoted by $J$. (Note: $J$ is distinct from the maximum rate of
  electron transport that could be achieved at _any_ incident PPFD, which is
  denoted by $J_{max}$.)
...
photorespiration and non-photorespiratory CO~2~ release; the rate of
non-photorespiratory CO~2~ release in the light is denoted by $R_L$. It is also
possible to calculate the net assimilation rates corresponding to each potential
...
inputs <- exdf(data.frame(
  Cc = seq(cc_step, cc_max, by = cc_step),
  Tleaf = Tleaf,
  total_pressure = 1,
  Oxygen = 21
))
...
assim <- calculate_c3_assimilation(inputs,
  alpha_g, alpha_old, alpha_s, alpha_t, Gamma_star, J, Kc, Ko, RL, Tp, Vcmax)
...
xyplot(
  ...
  panel = function(x, y, ...) {
    panel.lines(c(0, 0) ~ c(0, cc_max), lty = 2, col = 'black')
    panel.xyplot(x, y, ...)
  }
)
```
...
C~3~ leaf according to the FvCB model with
`alpha_old` = `r alpha_old`,
`J` = `r J` `r assim$units$J_tl`, `RL` = `r RL` `r assim$units$RL_tl`,
`Tp` = `r Tp` `r assim$units$Tp`, and
`Vcmax` = `r Vcmax` `r assim$units$Vcmax_tl`. Temperature response parameters
from @sharkey_fitting_2007 were used for these calculations.)
...
$J$, $J_{max}$, $R_L$, $T_p$, and $V_{cmax}$. See the documentation for
...
One point is that net assimilation models generally predict the response of
...
response to intercellular CO~2~ rather than the true response to chloroplastic
CO~2~.
...
was intentionally added to the data. Three curves exhibit a sharp and likely
...
by typing `?calculate_total_pressure` or `?calculate_total_pressure`.
...
files to calculate values of several key C~3~ photosynthetic parameters
(`r paste(names(c3_temperature_param_sharkey), collapse = ', ')`) according to
the temperature response parameters specified in @sharkey_fitting_2007.
...
Together, these functions add several new columns to `gasex_data`, including
`total_pressure`, `Gamma_star`, and others. With this information, we are now
ready to perform the fitting procedure.

For this operation, we can use the `fit_c3_aci` function from the `PhotoGEA`
package, which fits a single response curve to extract the values of key
photosynthetic parameters. To apply this function to each curve in a larger data
set and then consolidate the results, we can use it in conjunction with `by` and
`consolidate`, which are also part of `PhotoGEA`. (For more information about
these functions, see the built-in help menu entries by typing `?fit_c3_aci`,
...
we will vary all five key photosynthetic parameters: $\alpha_{old}$, $J$, $R_L$,
$T_p$, and $V_{cmax}$. The mesophyll conductance ($g_{mc}$) is set to infinity
by default, so these fits are made assuming $C_c = C_i$, and hence the estimates
of $V_{cmax}$ and other parameters should be considered as "effective" values
rather than chloroplastic values.
...
The `fit_c3_aci` function estimates values of $J$, but not $J_{max}$. However,
estimates of $J_{max}$ can be obtained after fitting by applying the
`calculate_jmax` function to the results from `fit_c3_aci`. For more information
about this function, see the built-in help menu entry by typing
`?calculate_jmax`.
...
With these commands, the `parameters` element of `c3_aci_results` now includes
estimates of $J_{max}$.
