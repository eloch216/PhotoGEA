...
the timestamps are interpreted correctly, we will also set the time zone to the
current system time zone. (The measurements were actually made in the US Central
time zone, but `tz = 'US/Central'` is not recognized by some operating systems.)
Since there are multiple files to read, we will call this function once for each
file using `lapply`:
...
licor_exdf_list <- lapply(licor_file_paths, function(fpath) {
  read_gasex_file(fpath, 'time', posix_options = list(tz = Sys.timezone()))
})
```
...
current system time zone. (The measurements were actually made in the US Central
time zone, but `tz = 'US/Central'` is not recognized by some operating systems.)
Since there are (potentially) multiple files to read, we will call this function
...
tdl_exdf_list <- lapply(tdl_file_paths, function(fpath) {
  read_gasex_file(fpath, 'TIMESTAMP', posix_options = list(tz = Sys.timezone()))
})
```
