...
[`plantecophys`](https://cran.r-project.org/web/packages/plantecophys/),
[`plantecowrap`](https://cran.r-project.org/web/packages/plantecowrap/),
and [`photosynthesis`](https://cran.r-project.org/web/packages/photosynthesis/))
...
the details of the function to be wrapped.
...
- In the plotting command above, we subtract `Rd` from `Ac`, `Aj`, and
  `Ap` because `plantecophys::fitaci` returns gross rates for these, but
  `Amodel` and `Ameas` are net assimilation rates. Subtracting respiration
  converts the gross rates to net rates.
...
)
{
  ...
  # `plantecophys::fitaci` returns gross rates for `Ac`, `Aj`, and `Ap`. For
  # consistency with `fit_c3_aci`, we convert them to net rates.
...
- The assimilation rates have also all been standardized to net rates, whereas
  `plantecophys::fitaci` returns some of them as gross rates by default.
